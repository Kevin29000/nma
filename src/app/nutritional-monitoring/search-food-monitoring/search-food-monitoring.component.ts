...
@Component({
  selector: 'app-search-food-monitoring',
  standalone: true,
  imports: [
    RouterOutlet,
    RouterLink,
  ...
  templateUrl: './search-food-monitoring.component.html',
  styles: []
})
export class SearchFoodMonitoringComponent implements OnInit {
  ...
  @Output() foodSelected = new EventEmitter<Food>(); // L'événement émis au parent
  ...
  openDetailFood(food: Food): void {    
    this.foodSelected.emit(food); // Émet l'aliment sélectionné au parent
  }
}
